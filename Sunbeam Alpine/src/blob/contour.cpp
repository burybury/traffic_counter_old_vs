...
namespace cvb
{
    ...
    void simplifyPolygonRecursive(CvContourPolygon const *p, int const i1, int const i2, bool *pnUseFlag,
        double const delta)
    {
        CV_FUNCNAME("cvSimplifyPolygonRecursive");
        __CV_BEGIN__;
        {
            size_t endIndex = (i2 < 0) ? p->size() : i2;
    ...
    CvContourPolygon *cvPolygonContourConvexHull(CvContourPolygon const *p)
    {
        ...
        __CV_BEGIN__;
        {
            ...
            for (unsigned int i = 3; i < p->size(); i++)
            {
                ...
                if ((cvCrossProductPoints((*p)[i], dq.at(0), dq.at(1)) >= 0) &&
                    (cvCrossProductPoints(dq.at(s - 2), dq.at(s - 1), (*p)[i]) >= 0))
    ...
    void cvWriteContourPolygonSVG(const CvContourPolygon &p, const string &filename, const CvScalar &stroke,
        const CvScalar &fill)
    {
        int minx = INT_MAX;
        int miny = INT_MAX;
        ...
        f <<
            ...
            maxx - minx << "px\" height=\"" << maxy - miny << "px\" viewBox=\"" << minx << " " << miny << " " << maxx <<
            " " << maxy << "\" zoomAndPan=\"disable\" >" << endl;

        f << "<polygon fill=\"rgb(" << fill.val[0] << "," << fill.val[1] << "," << fill.val[2] << ")\" stroke=\"rgb(" <<
            stroke.val[0] << "," << stroke.val[1] << "," << stroke.val[2] << ")\" stroke-width=\"1\" points=\"" <<
            buffer.str() << "\"/>" << endl;
