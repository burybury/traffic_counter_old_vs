...
namespace cvb
{
	...
	bool
	CvTrack::getArea(unsigned int* weightedArea) {
		if (foundBefore) {
			if (areaReadings.size() >= 5) {
				areaReadings.push_back(area);
				*weightedArea = 0;
				return false;
			}
			else {
				for each(unsigned int area in areaReadings) {
					*weightedArea += area;
				}

				//*weightedArea = *weightedArea / areaReadings.size();
				return true;
			}
		}
	...
	void cvRenderBlob(const IplImage *imgLabel, CvBlob *blob, IplImage *imgSource, IplImage *imgDest,
		unsigned short mode, CvScalar const &color, double alpha)
	{
		...
		if (mode & CV_BLOB_RENDER_COLOR)
		{
			...
			if (imgLabel->roi)
			{
				...
				imgLabel_offset =
					(imgLabel->nChannels * imgLabel->roi->xOffset) + (imgLabel->roi->yOffset * stepLbl);
			}
			if (imgSource->roi)
			{
				...
				imgSource_offset =
					(imgSource->nChannels * imgSource->roi->xOffset) + (imgSource->roi->yOffset * stepSrc);
			}
			if (imgDest->roi)
			{
			...
			unsigned char *source =
				(unsigned char *)imgSource->imageData + imgSource_offset + (blob->miny * stepSrc);
			unsigned char *imgData = (unsigned char *)imgDest->imageData + imgDest_offset + (blob->miny * stepDst);

			for (unsigned int r = blob->miny;
				r < blob->maxy; r++, labels += stepLbl, source += stepSrc, imgData += stepDst)
				for (unsigned int c = blob->minx; c < blob->maxx; c++)
				{
					if (labels[c] == blob->label)
					{
						imgData[imgDest->nChannels * c + 0] = (unsigned char)(
							(1. - alpha) * source[imgSource->nChannels * c + 0] + alpha * color.val[0]);
						imgData[imgDest->nChannels * c + 1] = (unsigned char)(
							(1. - alpha) * source[imgSource->nChannels * c + 1] + alpha * color.val[1]);
						imgData[imgDest->nChannels * c + 2] = (unsigned char)(
							(1. - alpha) * source[imgSource->nChannels * c + 2] + alpha * color.val[2]);
					}
				}
		}
		...
		if (mode)
		{
			if (mode & CV_BLOB_RENDER_TO_LOG)
			{
				...
				std::clog << " - Bounding box: (" << blob->minx << ", " << blob->miny << ") - (" << blob->maxx <<
					", " << blob->maxy << ")" << std::endl;
				std::clog << " - Bounding box area: " <<
					(1 + blob->maxx - blob->minx) * (1 + blob->maxy - blob->miny) << std::endl;
			...
			if (mode & CV_BLOB_RENDER_TO_STD)
			{
				...
				std::cout << " - Bounding box: (" << blob->minx << ", " << blob->miny << ") - (" << blob->maxx <<
					", " << blob->maxy << ")" << std::endl;
				std::cout << " - Bounding box area: " <<
					(1 + blob->maxx - blob->minx) * (1 + blob->maxy - blob->miny) << std::endl;
			...
			if (mode & CV_BLOB_RENDER_BOUNDING_BOX)
				cvRectangle(imgDest, cvPoint(blob->minx, blob->miny), cvPoint(blob->maxx - 1, blob->maxy - 1),
					CV_RGB(255., 0., 0.), 3);
			...
			{
				cvLine(imgDest, cvPoint(int(blob->centroid.x) - 3, int(blob->centroid.y)),
					cvPoint(int(blob->centroid.x) + 3, int(blob->centroid.y)), CV_RGB(0., 0., 255.));
				cvLine(imgDest, cvPoint(int(blob->centroid.x), int(blob->centroid.y) - 3),
					cvPoint(int(blob->centroid.x), int(blob->centroid.y) + 3), CV_RGB(0., 0., 255.));
			}
		}
...
#define _HSV2RGB_(H, S, V, R, G, B) \
  { \
	...
	void cvRenderBlobs(const IplImage *imgLabel, CvBlobs &blobs, IplImage *imgSource, IplImage *imgDest,
		unsigned short mode, double alpha)
	{
...
{
	output << b.label << ": " << b.area << ", (" << b.centroid.x << ", " << b.centroid.y << "), [(" << b.minx << ", " <<
